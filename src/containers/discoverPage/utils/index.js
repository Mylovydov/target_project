...
export const transformChartData = (dataToTransform, isOriginalSize, selectedOptions) => {
	if (!dataToTransform) {
		return;
	}
	...
	let datasets = dataToTransform.map(item => {
		const { count = 1, infoNeedId, searchTerm, infoNeedLabel, needProbability, axiosX, axiosY } = item;
		...
		const isSelectedItem = selectedOptions[infoNeed] === infoNeedLabel
			&& +selectedOptions[needId] === infoNeedId;

		const bubbleStyle = optionsHasValue
			? {
				backgroundColor: isSelectedItem ? backgroundColor : 'rgba(227,227,227,0.5)',

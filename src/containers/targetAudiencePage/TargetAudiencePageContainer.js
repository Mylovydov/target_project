...
const TargetAudiencePageContainer = () => {
	...
	const [, setAudienceFilter] = useLocalStorage(localStorageDefineTargetAudienceFilter);
	const [attributes, setAttributes] = useLocalStorage(localStorageDefineTargetAudienceAttrKey, null);
	...
	const handleChangeRangeAttr = useCallback((event, newValue, activeThumb) => {
		...
		setAttributes({ ...attributes, [rangeAttributesName]: changedAttributes });
	...
	const onSubmit = () => {
		...
		const audienceFilterItems = transformAttributesToFilter(attributesToTransform);
		...
		open({ message: 'Target audience definitions have been successfully applied', variant: 'success' });
	};

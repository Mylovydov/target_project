...
const RevealDialog = ({ open, onClose, selectedInfoNeedId, ...props }) => {
	...
	const onSubmitSearchTermSet = useCallback(
		() => {
			...
			const definition = audienceDefinitions.find(def => def.id === audienceFilter.id)

			if (definition) {
				const updatedDefinition = { ...definition, terms: [...definition.terms, ...selectedItems] }
				const newAudienceDefinitions = audienceDefinitions.map(
					def => def.id === definition.id ? updatedDefinition : def
				)

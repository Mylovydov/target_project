...
export const formFieldTypeHelperText = 'The field type is defined automatically based on the loaded data.'
export const formFieldLabelHelperText = 'The name of the field formed automatically on the basis of the loaded data. If the name is not defined correctly, you can change it. Avoid identical names'
export const formFieldCategoricalHelperText = 'A flag that indicates whether the field is categorical or not. Note that not all fields describing a respondent are categorical, but only fields such as age, education level, and similar. Fields with values 0-1 (boolean type) are NOT categorical'

// IDENTIFY FILTER HELPER TEXT
export const identifyFilterRangeHelperText = 'If you change the range, the table will contain search queries whose "Need Probability" parameter is included in the range. The default range is 0 to 100%';
export const identifyFilterTermNumberHelperText = 'Show top 100 (by default) search queries. If there are more than 100 search queries, then 100 will be displayed, but you can change the number of search queries displayed';
...
export const discoverInteractiveMapSubtitle = 'The closeness of the bubbles expresses the semantic similarity of the search terms, and the size of the bubbles indicates the number of identical search terms related to the same information needs\n';
export const discoverInteractiveMapLegendContainerId = 'legend-container';
export const discoverInteractiveMapDisableMessage = 'To display the interactive map, please specify X and Y coordinates in the downloaded file'
...
export const filterHelpText = 'Using filters, you will be able to select the desired searches by "Information Need" or "Need ID"';
export const editInfoNeedLabelHelperText = 'Click on the handle icon to change the name of the selected information need. You will then be able to export the file with the changes applied to all information needs ';
...
export const respondentAttributesHelperText = 'You can select the attributes of the target audience, to display statistics on the selected target audience';
...
export const addToSearchTermsSetHelperText = 'Add the selected search terms to the final set of search terms';
...
export const removeSelectedTermsHelperText = 'Remove selected search queries from the final set of queries';
export const exportSearchTermsSetHelperText = 'Export a set of search terms in csv format';

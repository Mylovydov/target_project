...
const BuildSetPageContainer = () => {
	...
	const [searchTermSet, setSearchTermSet] = useLocalStorage(
		localStorageSearchTermSetKey,
		[]
	);
	...
	const { filteredTerms, onChangeFilters, filterValues, setFilterValues } =
	...
	const onDeleteSearchTermsSetItems = useCallback(() => {
		const newTermSet = searchTermSet.filter(
			term => !selectedItems.includes(term)
		);
		const filteredAudienceDefinitions = audienceDefinitions.reduce(
			(acc, def) => {
				const filteredTerms = def.terms.filter(term =>
					newTermSet.includes(term)
				);
	...
	const onExportBtnClick = useCallback(() => {
		...
		const preparedDataToCSVExport = prepareDataToFinalExport(
			definitionsFilters,
			foundedSearchTerms
		);
		exportJsonOrCSVFile(
			preparedDataToCSVExport,
			CSVFormat.format,
			finalExportFileName
		);
		...
		<BuildSetPageStyled>
			...
			<BuildSetPageFooterStyled>
				<Stack direction="row" justifyContent="center" spacing={3}>
					<StepButton
						onClick={() => navigate(routes.revealInformation)}
						label="Previous step"
						color="warning"
					/>
				</Stack>

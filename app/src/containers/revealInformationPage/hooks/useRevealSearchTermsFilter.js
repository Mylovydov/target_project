...
const filterData = (itemsToFilter, filter) => {
	...
	const filteredSearchTerms = itemsToFilter.filter(searchTermItem =>
		checkFiltersMatching(
			{
...
const useRevealSearchTermsFilter = (
	defaultFilter,
	data,
	onFilteringComplete,
	debounceDelay = 300
	...
	const [filteredTerms, setFilteredSearchTerms] = useState(
		filterData(data, defaultFilter)
	);
			...
			switch (filterName) {
				case rangeProbabilityFilterName:
					...
					newFilterValue =
						activeThumb === 0
							? [
									Math.min(newValue[0], maxValue - minRangeDistanceOfRAINPage),
									maxValue
								]
							: [
									minValue,
									Math.max(newValue[1], minValue + minRangeDistanceOfRAINPage)
								];
					break;
	...
	const debouncedFilterTerms = useCallback(
		debounce(filterSearchTerms, debounceDelay, {
			trailing: true,
			leading: false
		}),
		[]

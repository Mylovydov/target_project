...
import {
  defineTypeDialogSubtitle,
  defineTypeDialogTitle,
  localStorageModeKey,
  routes,
} from "../../constants";
...
const MainPageContainer = () => {
  ...
  const onTestFileLoad = useCallback(async () => {
    try {
      ...
      const { default: demoRespondents } = await import(
        `../../assets/testdata_prepared.json`
      );
        ...
        <Stack spacing={4} direction="row" justifyContent="space-between">
          <UploadCard>
            <Stack spacing={3} flexGrow={1}>
              <DownloadFile
                title="To get a sample json, click on the link below"
                json={{
              ...
              <DropZone
                ...
                description="(Only .json file and sizes not exceeding 40Mb are accepted)"
              />
            </Stack>
          </UploadCard>
          <UploadCard>
